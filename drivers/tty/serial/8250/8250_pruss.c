 ...
 *
 *  Copyright (C) 2020 by Texas Instruments Incorporated - http://www.ti.com/
...
static int pruss8250_rs485_config(struct uart_port *port,
...
static int pruss8250_probe(struct platform_device *pdev)
{
	...
	info->pru = pru_rproc_get(np, 0);
	if (IS_ERR(info->pru)) {
		ret = PTR_ERR(info->pru);
		if (ret != -EPROBE_DEFER)
			dev_err(NULL, "failed to get pru (%d)\n", ret);
		return ret;
	}
	...
	up->rs485_config = pruss8250_rs485_config;
