...
/*** RTnet ***/
    ...
    lp->ltint = ltint;
    lp->mii = mii;
    if ((cards_found >= MAX_UNITS) || (options[cards_found] > (int)sizeof(options_mapping)))
...
static int
pcnet32_open(struct rtnet_device *dev) /*** RTnet ***/
{
...
static int
pcnet32_init_ring(struct rtnet_device *dev) /*** RTnet ***/
{
    struct pcnet32_private *lp = dev->priv;
    int i;
...
static int
pcnet32_start_xmit(struct rtskb *skb, struct rtnet_device *dev) /*** RTnet ***/
{
    struct pcnet32_private *lp = dev->priv;
    unsigned long ioaddr = dev->base_addr;
    u16 status;
    ...
    if (pcnet32_debug > 3) {
	rtdm_printk(KERN_DEBUG "%s: pcnet32_start_xmit() called, csr0 %4.4x.\n",
 ...
 *** RTnet ***/
	...
	if (pcnet32_debug > 5)
	    rtdm_printk(KERN_DEBUG "%s: interrupt  csr0=%#2.2x new csr=%#2.2x.\n",
	...
	if (csr0 & 0x0200) {		/* Tx-done interrupt */
		...
		if (status & 0x4000) {
			...
			rtdm_printk(KERN_ERR "%s: Tx FIFO error! CSR0=%4.4x\n",
			       dev->name, csr0);
			must_restart = 1;
		    }
#else
		...
		rtdm_printk(KERN_ERR "%s: out-of-sync dirty pointer, %d vs. %d, full=%d.\n",
	...
	if (csr0 & 0x0800) {
	    rtdm_printk(KERN_ERR "%s: Bus master arbitration failure, status %4.4x.\n",
		   dev->name, csr0);
...
static int
pcnet32_close(struct rtnet_device *dev) /*** RTnet ***/
{
    unsigned long ioaddr = dev->base_addr;
    struct pcnet32_private *lp = dev->priv;
    int i;
