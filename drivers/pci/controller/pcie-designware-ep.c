...
static int dw_pcie_ep_get_msi(struct pci_epc *epc)
{
	...
	dw_pcie_read_dbi(pci, pci->dbi_base, MSI_MESSAGE_CONTROL, 0x2, &val);
	val = (val & MSI_CAP_MME_MASK) >> MSI_CAP_MME_SHIFT;

	dw_pcie_read_dbi(pci, pci->dbi_base, MSI_MESSAGE_ADDR_L32, 0x4,
			 &lower_addr);
	dw_pcie_read_dbi(pci, pci->dbi_base, MSI_MESSAGE_ADDR_U32, 0x4,
			 &upper_addr);
