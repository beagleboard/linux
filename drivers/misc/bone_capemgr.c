...
static int bone_baseboard_scan(struct bone_baseboard *bbrd)
{
	...
	if (macc == NULL || macc->read == NULL) {
		dev_err(&info->pdev->dev,
			"No memory accessor for baseboard\n");
		return -ENODEV;
	}

	/* perform read */
	r = macc->read(macc, bbrd->signature, 0, sizeof(bbrd->signature));
	if (r != sizeof(bbrd->signature)) {
		if (r == -EPROBE_DEFER)
			return r;
		dev_info(&info->pdev->dev,
				"baseboard i2c read failed\n");
		return r >= 0 ? -EINVAL : r;
	}
		...
		char *buf)
{
	...
	list_for_each_entry(slot, &info->slot_list, node) {
		...
		len = sprintf(buf, "%2d: %02x:%c%c%c%c%c%c %3d %s\n",
				slot->slotno,
				(int)slot->client ?
		...
		const char *part_number, const char *version, int prio)
{
	...
	if (node) {
		...
		if (IS_ERR_OR_NULL(slot->macc)) {
			dev_err(dev, "slot #%d: failed to get memory accessor\n",
				slotno);
...
capemgr_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(bbrd->macc)) {
		dev_err(&pdev->dev, "Failed to get baseboard memory accessor\n");
