...
static void pa_tx_compl_work_handler(unsigned long data)
{
	...
	for (; ;) {
		...
		p_info = (struct pa_packet *)KNAV_DMA_GET_SW_DATA0(hwdesc);
		if (!p_info) {
			dev_err(core_dev->dev,
				  ...
				  struct knav_dma_desc *desc)
{
	...
	while (dma_desc) {
		...
		org_buf_ptr = (void *)KNAV_DMA_GET_SW_DATA0(ndesc);
		org_buf_len = (unsigned int)KNAV_DMA_GET_SW_DATA1(ndesc);
...
static void pa_rx_compl_work_handler(unsigned long data)
{
	...
	for (; ;) {
		...
		org_buf_ptr = (void *)KNAV_DMA_GET_SW_DATA0(hwdesc);
		org_buf_len = (unsigned int)KNAV_DMA_GET_SW_DATA1(hwdesc);
		knav_dma_get_pkt_info(&dma_buf, &buf_len, &dma_desc, hwdesc);
		dma_unmap_single(core_dev->dev, dma_buf, org_buf_len,
				 DMA_TO_DEVICE);
				 ...
				 unsigned int cluster_id)
{
	 ...
	 * field as a 32bit value. Will not work on 64bit machines
	 */
	KNAV_DMA_SET_SW_DATA0((u32)p_info, p_info->hwdesc);
