...
struct pa_intf {
	...
	unsigned int		data_flow_num;
	unsigned int		data_queue_num;
...
struct pa_cluster_config {
	...
	unsigned int		tx_cmd_cmpl_queue_id;
	struct pa_core_device	*core_dev;
...
struct pa_hw {
	...
	int (*fmtcmd_align)(struct netcp_packet *p_info,
	...
	int	(*add_mac_rule)(struct pa_intf *pa_intf, int index,
				...
				int rule, unsigned int etype, int port);
...
struct pa_core_device {
	...
	unsigned int rx_flow_base;
	unsigned int rx_queue_base;
	...
	unsigned int cmd_flow_num;
	unsigned int cmd_queue_num;
