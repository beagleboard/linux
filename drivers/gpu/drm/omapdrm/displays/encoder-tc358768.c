...
static int tc358768_write(struct tc358768_drv_data *ddata, u32 reg, u32 val)
{
	int r;
...
static int tc358768_read(struct tc358768_drv_data *ddata, u32 reg, u32 *val)
{
	int r;
	u32 v1, v2;
...
static int tc358768_calc_pll(struct tc358768_drv_data *ddata)
{
	...
	target_pll = tc358768_pclk_to_pll(ddata, ddata->vm.pixelclock);
...
{
	const struct videomode *vm = &ddata->vm;
	...
	/* vsw (+ vbp) */
	tc358768_write(ddata, TC358768_DSI_VSW, vm->vsync_len + vm->vback_porch);
	/* vbp (not used in event mode) */
	tc358768_write(ddata, TC358768_DSI_VBPR, 0);
	/* vact */
	tc358768_write(ddata, TC358768_DSI_VACT, vm->vactive);

	/* (hsw + hbp) * byteclk * ndl / pclk */
	tc358768_write(ddata, TC358768_DSI_HSW,
		(u32)div_u64((vm->hsync_len + vm->hback_porch) * ((u64)ddata->bitclk / 4) * ddata->dsi_ndl, vm->pixelclock));
	/* hbp (not used in event mode) */
	tc358768_write(ddata, TC358768_DSI_HBPR, 0);
	/* hact (bytes) */
	tc358768_write(ddata, TC358768_DSI_HACT, vm->hactive * 3);
				 ...
				 struct videomode *vm)
{
	...
	ddata->vm.flags &= ~DISPLAY_FLAGS_PIXDATA_POSEDGE;
	ddata->vm.flags |= DISPLAY_FLAGS_PIXDATA_NEGEDGE;

	ddata->vm.flags &= ~DISPLAY_FLAGS_SYNC_NEGEDGE;
	ddata->vm.flags |= DISPLAY_FLAGS_SYNC_POSEDGE;
