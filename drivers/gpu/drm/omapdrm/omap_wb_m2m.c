			  ...
			  const struct omap_dss_writeback_info *wb_info)
{
	...
	r = dispc_ovl_setup(src_plane, src_info, 0, &t, 1);
	if (r)
		return false;

	dispc_ovl_set_channel_out(src_plane, OMAP_DSS_CHANNEL_WB);
	dispc_ovl_enable(src_plane, true);
...
static void device_run(void *priv)
{
	...
	src_dma_addr[0] = vb2_dma_addr_plus_data_offset(src_vb, 0);
	if (spix->num_planes == 2)
		src_dma_addr[1] = vb2_dma_addr_plus_data_offset(src_vb, 1);
	if (!src_dma_addr[0]) {
		log_err(dev,
			"acquiring source buffer(%d) dma_addr failed\n",
			src_vb->v4l2_buf.index);
		return;
	}

	dpix = &d_q_data->format.fmt.pix_mp;
	dst_dma_addr[0] = vb2_dma_addr_plus_data_offset(dst_vb, 0);
	if (dpix->num_planes == 2)
		dst_dma_addr[1] = vb2_dma_addr_plus_data_offset(dst_vb, 1);
	if (!dst_dma_addr[0]) {
		log_err(dev,
...
static int wbm2m_stop_streaming(struct vb2_queue *q)
{
	struct wbm2m_ctx *ctx = vb2_get_drv_priv(q);
	...
	if (V4L2_TYPE_IS_OUTPUT(q->type)) {
		 ...
		 */
		dispc_ovl_enable(omap_plane_id(ctx->dev->plane),
					    false);
		dispc_ovl_enable(OMAP_DSS_WB, true);
